class EventsManager {
    ...
    obtenerDataInicial() {
        ...
        form_data.append('user', location.href.split('?')[1].split('=')[1])
        $.ajax({
          url: url,
          dataType: "json",
          data:form_data,
    ...
    poblarCalendario(eventos) {
        var def = new Date();
        var pad = "00";
        def = def.getFullYear() + '-' + (pad.substring(0, pad.length - def.getMonth().length) + (def.getMonth() + 1)) + '-' + (pad.substring(0, pad.length - def.getDate().length) + def.getDate());
